...
void Foam::Time::setFaceCells(const myLabelList* cells)
{
	//myLabelList* tempPtr = const_cast<myLabelList*> (cells);
	label tSize = cells->size();
	myLabelList* list = commProfiler_.curTimeSecPtr()->getProcFaceCellsPtr();
	if( list == NULL)
	{
//		Foam::Pout<< "************cells->size()="<< tSize<<"********  cells: " << *cells << "***********" << Foam::endl;
		//Foam::Pout<<cells->size()<<"33ssssssssssssssssssssssssssssssssssss----------------------\n\n";
		//commProfiler_.curTimeSecPtr()->setProcFaceCellsPtr(cells);
		commProfiler_.curTimeSecPtr()->setProcFaceCellsPtr(new myLabelList(tSize));
		commProfiler_.curTimeSecPtr()->getProcFaceCellsPtr()->assign(*cells);
	}
	else
	{
		Foam::Pout<< "************cells->size()="<< tSize<<"********  cells: " << *cells << "***********" << Foam::endl;
		commProfiler_.curTimeSecPtr()->setProcFaceCellsPtr(new myLabelList(tSize));
		commProfiler_.curTimeSecPtr()->getProcFaceCellsPtr()->append(*cells);
	}
}
//add by Xiaowei:end
